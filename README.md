...
A production-style REST API built with **FastAPI** and **SQLAlchemy**, connected to a cloud-hosted **PostgreSQL** database (Neon.tech). Provides full CRUD for products, read access to user/order data, and API-key protected write endpoints.
...
## ✨ Features
- **Full CRUD for products** (`GET`, `POST`, `PUT`, `DELETE`). `users` and `orders` are read-only through this API — they're populated by a separate SQL ETL pipeline ([sql-ecommerce-pipeline](https://github.com/Vlad34745/sql-ecommerce-pipeline)) and exposed here as a nested, read-only `/users/{id}/orders` lookup
- **API versioning:** all endpoints live under `/api/v1/...`, so a future breaking change can ship as `/api/v2` without affecting existing clients
- **Nested relational data:** `/api/v1/users/{id}/orders` returns each order together with its full product details in a single response, using SQLAlchemy `relationship()`
...
- **Pagination:** `GET /api/v1/products` supports `skip`/`limit` query params (capped at 500 per page) to avoid dumping the whole table in one response
- **CORS enabled** so a separate frontend (different origin/port) can call this API directly from the browser
- **Fails fast on misconfiguration:** the app refuses to start with a clear error message if `DATABASE_URL` or `API_KEY` is missing, instead of crashing later with a confusing stack trace
...
- **Linting (Ruff) in CI** on every push/PR, plus **Dependabot** for weekly, grouped dependency updates
- **MIT licensed**
...
|---|---|---|---|
| GET | `/` | No | Health check |
| GET | `/api/v1/products?skip=0&limit=100` | No | List products (paginated) |
| GET | `/api/v1/products/{product_id}` | No | Get a single product by ID |
| POST | `/api/v1/products` | Yes | Create a new product |
| PUT | `/api/v1/products/{product_id}` | Yes | Update an existing product |
| DELETE | `/api/v1/products/{product_id}` | Yes | Delete a product (blocked if it has linked orders) |
| GET | `/api/v1/users/{user_id}/orders` | No | List all orders for a user, with full product details nested in each order |
...
├── .github/workflows/tests.yml             # CI: lint + tests + coverage upload to Codecov
├── .github/dependabot.yml                    # Weekly, grouped dependency update PRs
├── .env.example                                # Template for required environment variables
├── requirements.txt                              # Production dependencies (pinned versions)
└── requirements-dev.txt                            # Adds pytest, pytest-cov, httpx, ruff
```# E-Commerce REST API
