...
def test_create_product_with_wrong_api_key_is_rejected(client):
    ...
    response = client.post(
        "/api/v1/products", json=payload, headers={"X-API-Key": "totally-wrong-key"}
    )
    assert response.status_code == 401
...
def test_delete_nonexistent_product_returns_404(client, auth_headers):
    response = client.delete("/api/v1/products/999999", headers=auth_headers)
    assert response.status_code == 404
...
def test_pagination_limits_results(client, auth_headers):
    ...
    response = client.get("/api/v1/products", params={"skip": 4, "limit": 2})
    assert response.status_code == 200
